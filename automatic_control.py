...
class CameraManager(object):
    ...
    def add_sensor(self, sensor_info):
        ...
        for s in sensor_info:
            ...
            if sensor_type == 'sensor.camera.rgb':
                ...
                converter = cc.Raw # Or cc.Depth if you want grayscale visualization
            else:
                converter = None # Lidar/Other

            sensor = [sensor_type, converter, s['id']]
    ...
    def update_est_trajectories(self,latest_pose):
        ...
        '''
        # 'latest_pose' from odometry.py is already the full world pose
        # relative to frame 0. No transformation is needed.

        if latest_pose is not None:
            # Extract position and rotation
            world_pos = latest_pose[:3] # [x,y,z]
            world_rot_rad = latest_pose[3:] # [roll(rad), pitch(rad), yaw(rad)]

            # Convert euler (radians) to quaternion [x, y, z, w]
            # as required by the 'Rotation' library
            quat_xyzw = Rotation.from_euler('xyz', world_rot_rad).as_quat()

            # Reorder quaternion to [w, x, y, z] for ate.py
            world_quat_wxyz = np.array([quat_xyzw[3], quat_xyzw[0], quat_xyzw[1], quat_xyzw[2]])

            # Append to the list
            self.est_trajectories.append(np.hstack((world_pos, world_quat_wxyz)).reshape((7,1)))
...
def game_loop(args):
    ...
    try:
        ...
        while True:
            ...
            # Only update trajectories if the agent has a valid pose estimate
            if agent.latest_pose is not None:
                world.camera_manager.get_gt_pose()
                world.camera_manager.update_est_trajectories(agent.latest_pose)
